...
class AladynSurvivalModeldiffKernelsavgloss(nn.Module):
    def __init__(self, N, D, T, K, P, G, Y, prevalence_t):
        ...
        # Generate random length scales within reasonable bounds
        lambda_scales = np.random.uniform(T/10, T/2, K)
        phi_scales = np.random.uniform(T/10, T/2, K)
        
        # Initialize separate kernel parameters for lambda and phi
        self.lambda_length_scales = nn.Parameter(
            torch.tensor(lambda_scales, dtype=torch.float32)
        )
        self.lambda_log_amplitudes = nn.Parameter(
            torch.zeros(K, dtype=torch.float32)  # Start with amplitude = 1
        )
        
        self.phi_length_scales = nn.Parameter(
            torch.tensor(phi_scales, dtype=torch.float32)
        )
    ...
    def update_kernels(self):
        """Update covariance matrices with separate parameters for lambda and phi"""
        ...
        for k in range(self.K):
            # Lambda kernel
            lambda_length = torch.clamp(self.lambda_length_scales[k], 
                                      min=max(1.0, self.T/20), 
                                      max=self.T/2)
            lambda_amp = torch.exp(torch.clamp(self.lambda_log_amplitudes[k], 
                                             min=-2.0, max=1.0))
            
            # Phi kernel
            phi_length = torch.clamp(self.phi_length_scales[k], 
                                   min=max(1.0, self.T/20), 
                                   max=self.T/2)
            phi_amp = torch.exp(torch.clamp(self.phi_log_amplitudes[k], 
                                          min=-2.0, max=1.0))
            
            # Compute kernels separately
            K_lambda = lambda_amp ** 2 * torch.exp(-0.5 * sq_dists / (lambda_length ** 2))
            K_phi = phi_amp ** 2 * torch.exp(-0.5 * sq_dists / (phi_length ** 2))
            ...
            for K, name in [(K_lambda, 'lambda'), (K_phi, 'phi')]:
                ...
                while True:
                    ...
                    jitter *= 2
                    if jitter > 0.1:
                        print(f"Warning: Large jitter needed for {name} kernel, topic {k}")
                        break
                
                if name == 'lambda':
                    self.K_lambda.append(K + jitter * torch.eye(self.T))
                else:
                    self.K_phi.append(K + jitter * torch.eye(self.T))
...
# Result: contributes -log(1-pi[n,d,5]) to loss
    ...
    def fit(self, event_times, num_epochs=1000, learning_rate=1e-3, lambda_reg=1e-2,
        ...
        """
        # Initialize optimizer with separate kernel parameters
        optimizer = optim.Adam([
            {'params': [self.lambda_, self.phi]},
            {'params': [self.lambda_length_scales, self.lambda_log_amplitudes,
                       self.phi_length_scales, self.phi_log_amplitudes]},
            {'params': [self.gamma], 'weight_decay': lambda_reg}
        ], lr=learning_rate)
        
        # Initialize tracking with separate kernel parameters
        history = {
            'loss': [],
            'lambda_length_scales': [],  # Changed from length_scales
            'lambda_amplitudes': [],     # Changed from amplitudes
            'phi_length_scales': [],     # New
            'phi_amplitudes': [],        # New
            'max_grad_lambda': [],
            'max_grad_phi': [],
            'max_grad_gamma': [],
            'condition_number_lambda': [],  # Changed from condition_number
            'condition_number_phi': []      # New
        }
        ...
        for epoch in range(num_epochs):
            ...
            # Update non-gradient history
            history['lambda_length_scales'].append(self.lambda_length_scales.detach().numpy().copy())
            history['lambda_amplitudes'].append(torch.exp(self.lambda_log_amplitudes).detach().numpy().copy())
            history['phi_length_scales'].append(self.phi_length_scales.detach().numpy().copy())
            history['phi_amplitudes'].append(torch.exp(self.phi_log_amplitudes).detach().numpy().copy())
            ...
            # Track condition numbers separately for lambda and phi kernels
            lambda_conds = []
            phi_conds = []
            for k in range(self.K):
                lambda_conds.append(torch.linalg.cond(self.K_lambda[k]).item())
                phi_conds.append(torch.linalg.cond(self.K_phi[k]).item())
    ...
    def _print_progress(self, epoch, loss, history):
        ...
        print(f"Lambda length scales: {self.lambda_length_scales.detach().numpy()}")
        print(f"Phi length scales: {self.phi_length_scales.detach().numpy()}")
        print(f"Lambda amplitudes: {torch.exp(self.lambda_log_amplitudes).detach().numpy()}")
        print(f"Phi amplitudes: {torch.exp(self.phi_log_amplitudes).detach().numpy()}")
...
def plot_training_diagnostics(history):
    """Plot training diagnostics with separate kernel parameters"""
    ...
    # Plot length scales
    plt.subplot(2, 2, 2)
    lambda_scales = np.array(history['lambda_length_scales'])
    phi_scales = np.array(history['phi_length_scales'])
    
    for k in range(lambda_scales.shape[1]):
        plt.plot(lambda_scales[:, k], '--', label=f'λ Topic {k}')
        plt.plot(phi_scales[:, k], ':', label=f'φ Topic {k}')
    plt.title('Length Scales Over Time')
    plt.xlabel('Epoch')
