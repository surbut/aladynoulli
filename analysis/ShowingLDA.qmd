...
We generate Data with 100 individuals, 5 diseases, 10 time points, and 3 topics (2 genetic features).
I show the induced sparsity (max mu_d = 0.02 in this simulation)
```{r, warning=FALSE, message=FALSE}
library(topicmodels)
library(ggplot2)
source("../Rscripts/simulations/newsim.R")
...
1. Summing across the time dimension (3rd dimension)
#### Collapse Y along the time dimension to get a matrix of individuals x diseases
 We will have to eliminate folks who have no disease (i.e. rowSums(Y_summed) == 0)
 We can then visualize the data to ensure that we have a count matrix, and plot our average $theta_ik$ and $eta_kd$ values. Recall that for a given topic our $eta_kd$ values need NOT sum to 1 across diseases.
